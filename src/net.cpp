...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Kazcoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"kazcointools.com", "95.151.209.19"},
    {NULL, NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Kazcoin is probably already running."), addrBind.ToString().c_str());
        else
